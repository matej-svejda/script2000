...
{
public:
	ParseTableCalculation(const std::vector<std::vector<std::string>>& rules, bool print_debug = false);
...
private:
	...
	std::unordered_map<std::string, std::vector<int>> non_terminal_to_rule_indices_;
	std::unordered_map<std::string, std::set<std::string>> non_terminal_first_sets_;
};
